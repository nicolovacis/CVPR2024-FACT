#!/usr/bin/env python3
"""
Multi-label Preprocessing script for Stanford dataset to FACT format (BALANCED)
...
def convert_labels_to_multilabel(label_file, num_seconds):
    ...
    print(f"      Multi-label statistics (BEFORE balancing):")
...
def balance_multilabel_dataset(features, multilabels, seed=42):
    """
    Balance dataset by downsampling background to match the max of other classes.
    
    For multi-label, we define 4 categories:
    - background (neither licking nor shaking): [0, 0]
    - licking only: [1, 0]
    - shaking only: [0, 1]
    - both: [1, 1]
    
    We downsample background to match the largest of the other 3 classes.
    """
    np.random.seed(seed)
    
    # Identify each category
    licking_only_mask = (multilabels[:, 0] == 1) & (multilabels[:, 1] == 0)
    shaking_only_mask = (multilabels[:, 0] == 0) & (multilabels[:, 1] == 1)
    both_mask = (multilabels[:, 0] == 1) & (multilabels[:, 1] == 1)
    background_mask = (multilabels[:, 0] == 0) & (multilabels[:, 1] == 0)
    
    n_licking_only = np.sum(licking_only_mask)
    n_shaking_only = np.sum(shaking_only_mask)
    n_both = np.sum(both_mask)
    n_background = np.sum(background_mask)
    
    print(f"      ───────────────────────────────────────────────────────", flush=True)
    print(f"      BALANCING DATASET", flush=True)
    print(f"      ───────────────────────────────────────────────────────", flush=True)
    print(f"      Original distribution:")
    print(f"        Licking only: {n_licking_only}")
    print(f"        Shaking only: {n_shaking_only}")
    print(f"        Both: {n_both}")
    print(f"        Background: {n_background}")
    sys.stdout.flush()
    
    # Find max of non-background classes
    max_class_count = max(n_licking_only, n_shaking_only, n_both)
    
    print(f"      Max non-background class count: {max_class_count}")
    print(f"      Downsampling background from {n_background} to {max_class_count}")
    sys.stdout.flush()
    
    # Get indices for each category
    licking_only_idx = np.where(licking_only_mask)[0]
    shaking_only_idx = np.where(shaking_only_mask)[0]
    both_idx = np.where(both_mask)[0]
    background_idx = np.where(background_mask)[0]
    
    # Downsample background
    if n_background > max_class_count:
        sampled_background_idx = np.random.choice(background_idx, size=max_class_count, replace=False)
    else:
        sampled_background_idx = background_idx
    ...
    balanced_indices = np.concatenate([
        ...
        shaking_only_idx,
        both_idx,
        sampled_background_idx
    ])
    
    # Shuffle
    np.random.shuffle(balanced_indices)
    
    # Apply to features and labels
    balanced_features = features[balanced_indices]
    balanced_multilabels = multilabels[balanced_indices]
    
    # Final statistics
    final_licking_only = np.sum((balanced_multilabels[:, 0] == 1) & (balanced_multilabels[:, 1] == 0))
    final_shaking_only = np.sum((balanced_multilabels[:, 0] == 0) & (balanced_multilabels[:, 1] == 1))
    final_both = np.sum((balanced_multilabels[:, 0] == 1) & (balanced_multilabels[:, 1] == 1))
    final_background = np.sum((balanced_multilabels[:, 0] == 0) & (balanced_multilabels[:, 1] == 0))
    
    print(f"      Balanced distribution:")
    print(f"        Licking only: {final_licking_only}")
    print(f"        Shaking only: {final_shaking_only}")
    print(f"        Both: {final_both}")
    print(f"        Background: {final_background}")
    print(f"      Total samples after balancing: {len(balanced_features)}")
    print(f"      ───────────────────────────────────────────────────────", flush=True)
    sys.stdout.flush()
    
    return balanced_features, balanced_multilabels


def create_mapping_file(mapping_file):
    """Create mapping.txt file with multi-label format"""
    with open(mapping_file, 'w') as f:
        f.write("# Multi-label format (BALANCED): each second has 2 independent binary labels\n")
        f.write("# Dataset balanced by downsampling background\n")
        f.write("0 licking\n")
        f.write("1 shaking\n")
...
def main():
    parser = argparse.ArgumentParser(description='Preprocess Stanford dataset for FACT (Multi-label BALANCED: licking & shaking)')
    ...
    print("=" * 80, flush=True)
    print("STANFORD MULTI-LABEL DATASET PREPROCESSING (ORIGINAL FPS, BALANCED)", flush=True)
    ...
    print(f"  BALANCED - Downsampling background to match other classes", flush=True)
    sys.stdout.flush()
    ...
    for video_idx, video_file in enumerate(video_files, 1):
        ...
        try:
            print(f"[{video_id}] Step 1/4: Extracting features...", flush=True)
        ...
        try:
            print(f"[{video_id}] Step 2/4: Converting to multi-label format...", flush=True)
        ...
        # Balance dataset
        try:
            print(f"[{video_id}] Step 3/4: Balancing dataset...", flush=True)
            features, multilabels = balance_multilabel_dataset(features, multilabels, seed=args.seed)
            print(f"[{video_id}]   After balancing: {features.shape}")
    ...
    print(f"  - BALANCED: Background downsampled to match other classes")
